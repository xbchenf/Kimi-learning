...
# 三、模型下载
## （一）安装modelscope
检查指令`pip show modelscope`；安装指令：`pip show modelscope` （Version: 1.26.0）
...
模型总体大小31G左右，下载耗时30分钟左右

# 四、前端代码
前端部分主要实现用户界面展示、用户输入处理以及与后端 API 的交互等功能。以下是前端的核心代码片段（完整代码可根据实际项目结构进行补充完善）：
```html
<!DOCTYPE html>
<html lang="zh">
<head>
...
# 五、后端代码
后端采用 Flask 框架，主要实现与前端的交互、模型推理等功能。chat_app.py完整代码如下：
```python
...
MODEL_ID = "/root/autodl-tmp/moonshotai/Kimi-VL-A3B-Instruct"  # 替换为实际的模型路径或名称
tokenizer = None
model = None
processor = None
# 用于存储对话历史的字典
...
> 注意：记得将 MODEL_ID 修改为自己的模型下载路径。


# 六、应用启动
完成上述搭建后，即可运行应用。在终端中导航到项目目录，运行以下命令启动 Flask 应用：
```
python chat_app.py
```

应用将在 http://localhost:5000 上运行。启动后模型会在后台自动加载，这可能需要30秒左右。在此期间，界面会显示 “模型正在加载中” 的提示，加载完成后，显示“模型加载完成”才能开始对话。


# 七、对话体验
在浏览器中打开：http://localhost:5000

先点击“上传图片” 上传需要进行分析的图片，然后在输入框中输入您的问题。点击 “发送” 按钮即可与本地模型进行对话。
对话体验截图效果如下：
